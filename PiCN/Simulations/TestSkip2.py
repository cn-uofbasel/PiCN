...
class TestSkip2(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.car = ICNForwarder(0, encoder=self.encoder_type, routing=True, interfaces=[self.simulation_bus.add_interface("car")])
        self.fetch_tool_car = Fetch("car", None, 255, self.encoder_type, interfaces=[self.simulation_bus.add_interface("ftcar")])
        ...
        for i in range(4):
            self.rsus.append(NFNForwarderData(0, encoder=self.encoder_type, interfaces=[self.simulation_bus.add_interface(f"rsu{i}")],
                                              chunk_size=chunk_size, num_of_forwards=2))
            self.fetch_tools.append(Fetch(f"rsu{i}", None, 255, self.encoder_type,
                                     interfaces=[self.simulation_bus.add_interface(f"ft{i}")]))
            self.rsus[i].nfnlayer.optimizer = EdgeComputingOptimizer(self.rsus[i].icnlayer.cs, self.rsus[i].icnlayer.fib,
    ...
    def setup_faces_and_connections(self):
        ...
        self.d = Content(Name("/car/data/test1"), "Test" * 100)
    ...
    def test_nfn_interest_skip2(self):
        ...
        res2 = self.fetch_tools[3].fetch_data(name, timeout=5)
        print(res2)
